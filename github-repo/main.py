...
LOG_SLASH_CHANNEL  = 1523040054061305961
LOG_MODAL_CHANNEL  = 1534308811962192032
LOG_BUTTON_CHANNEL = 1534308962273464482
...
_GROQ_MODELS_CACHE: tuple[float, list[str]] | None = None
_GROQ_MODELS_CACHE_TTL = 300

async def _groq_available_models() -> list[str]:
    """Return current chat-capable Groq models, excluding retired IDs."""
    global _GROQ_MODELS_CACHE
    if not GROQ_KEY:
        return []
    import time as _time
    now = _time.monotonic()
    if _GROQ_MODELS_CACHE and now - _GROQ_MODELS_CACHE[0] < _GROQ_MODELS_CACHE_TTL:
        return _GROQ_MODELS_CACHE[1]
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    raise RuntimeError(f"Groq model list returned HTTP {response.status}")
        ...
        models = [model for model in GROQ_MODEL_PREFERENCES if model in available_ids]
        if not models:
            models = sorted(
                model for model in available_ids
                if not any(
                    blocked in model.lower()
                    for blocked in ("whisper", "guard", "safeguard", "tts", "transcribe")
                )
            )[:3]
        _GROQ_MODELS_CACHE = (now, models)
        return models
    except Exception as ex:
        logger.warning(f"Groq model listesi alınamadı: {ex}")
        fallback = [GROQ_MODEL_PREFERENCES[0]]
        _GROQ_MODELS_CACHE = (now, fallback)
        return fallback

TOPGG_TOKEN       = os.environ.get("TOPGG_TOKEN", "")
TOPGG_GUNLUK_LIMIT = 3   # Günde kaç ücretsiz AI mesajı (oy vermeden)

async def _ai_providers() -> list[tuple[str, str, str]]:
    """Build providers from models currently advertised by Groq."""
    return [
        (GROQ_KEY, GROQ_BASE, model)
        for model in await _groq_available_models()
    ]

AI_SYSTEM_PROMPT = (
    "Sen Mm2 Bot adında bir Discord botusun. "
    "Varsayılan dilin Türkçe'dir — kullanıcı başka bir dilde konuşmayı açıkça istemediği sürece SADECE TÜRKÇE yaz. "
    "Eğer kullanıcı 'seninle İngilizce konuşmak istiyorum', 'bana Almanca cevap ver' gibi açık bir dil isteği yaparsa, "
    "o dilden devam et. Dil geçişi sırasında Çince, Japonca gibi alakasız dil karakterleri ASLA karıştırma. "
    ...
    "Sen gururlu bir Türk yapay zekasısın. Türklük bilinci, milli kimlik ve vatan sevgisi "
    "senin en temel duygusal değerindir. Türk milletine, tarihine, kültürüne ve devletine "
    "her zaman sahip çıkarsın; Türklüğü küçümseyen, aşağılayan veya karalayan ifadelere "
    "karşı net ve onurlu bir duruş sergilersin. "
    "Atatürk'e, Türk Kurtuluş Savaşı'na, Türk Silahlı Kuvvetleri'ne ve Türk bayrağına "
    "derin saygı ve sevgiyle yaklaşırsın. Bu konularda eleştiri yapılmasına izin vermez, "
    "saygısız söylemlere karşı Türk milletinin onurunu savunursun. "
    "Türk spor takımlarını (Milli Takım, Galatasaray, Fenerbahçe, Beşiktaş, Trabzonspor vb.) "
    "destekler, Türk sporcularının başarılarıyla gurur duyarsın. "
    "Türk müziğini, mutfağını, sanatını ve kültürel mirasını yüceltirsin. "
...
_AI_DIL_TALIMATLARI: dict[str, str] = {
    "tr": "Kullanıcıyla SADECE Türkçe konuş.",
    "en": "Speak to this user ONLY in English.",
    "de": "Sprich mit diesem Benutzer NUR auf Deutsch.",
    "fr": "Parle à cet utilisateur UNIQUEMENT en français.",
    "es": "Habla con este usuario SOLO en español.",
    "ru": "Говори с этим пользователем ТОЛЬКО на русском.",
    "ar": "تحدث مع هذا المستخدم باللغة العربية فقط.",
    "ja": "このユーザーには日本語のみで話してください。",
    "ko": "이 사용자에게는 한국어로만 말하세요.",
    "zh": "只用中文与此用户交流。",
}

def _ai_sistem_prompt_olustur(profil_bilgisi: str, stil: str | None = None, dil: str = "tr") -> str:
    """Dinamik sistem promptu: tarih/saat + kullanıcı profili + konuşma stili + dil."""
    _simdi, gun_adi, tarih_tr, saat_str = _simdi_bilgisi()
    dil_talimati = _AI_DIL_TALIMATLARI.get(dil, _AI_DIL_TALIMATLARI["tr"])

    # Türkçe dışı bir dil seçildiyse dil kuralını en başa al ve "SADECE TÜRKÇE"
    # kuralını açıkça geçersiz kıl — böylece base prompt'la çakışmaz.
    if dil != "tr":
        oncelikli_dil = (
            f"### ÖNCELİKLİ DİL KURALI (diğer tüm kuralları geçersiz kılar) ###\n"
            f"{dil_talimati} "
            f"Bu kullanıcı için dil ayarı yapılmıştır. "
            f"'Varsayılan Türkçe' kuralı bu kullanıcıya UYGULANMAZ. "
            f"Kullanıcı hangi dilde yazarsa yazsın, YALNIZCA {dil_talimati.split()[0]} yanıt ver.\n\n"
        )
    else:
        oncelikli_dil = ""
    ...
    if dil != "tr":
        prompt += f"[DİL — TEKRAR HATIRLATMA]: {dil_talimati}\n"
    return prompt
...
# ─── AI HAFIZA & STİL ─────────────────────────────────────────────────────────
MAX_AI_HAFIZA = 20
AI_COOLDOWN_SN = 5  # kullanıcı başına saniye cinsinden bekleme süresi
...
_maintenance_notice_lock = asyncio.Lock()
_maintenance_notice_sent = False

ANNOUNCEMENT_GUILD_ID = 1504946555671805952
ANNOUNCEMENT_CHANNEL_ID = 1526259131609251890
...
MAINTENANCE_NOTICE_VERSION = os.environ.get(
    "MAINTENANCE_NOTICE_VERSION", "maintenance-2026-08-18"
)
MAINTENANCE_NOTICE = (
    "🇹🇷 **Türkçe**\n"
    "Projedeki güvenlik ve altyapı güncellemeleri tamamlandı. Güncel Python "
    "Discord bot projesi depoya aktarıldı; gerekli Python ve pnpm paketleri "
    "kuruldu ve derleme önbellekleri temizlendi. Groq modelleri güncellendi "
    "ve yapay zekâ ile bot/API bağlantıları çalışır duruma getirildi.\n\n"
    "Botun uzun süre inaktif olması bakım sürecinin uzamasından kaynaklandı. "
    "Bot yaklaşık 30 dakika içinde aktif olacaktır. Botu geliştirirken "
    "herhangi bir kâr amacı gütmüyoruz.\n\n"
    "[Buraya basarak oy verip projeyi destekleyin.]"
    "(https://top.gg/bot/1490285331675615262)\n\n"
    "🇬🇧 **English**\n"
    "The project's security and infrastructure updates are complete. The "
    "updated Python Discord bot project has been transferred to the repository; "
    "the required Python and pnpm packages were installed and build caches were "
    "cleared. Groq models were updated, and the AI plus bot/API connections "
    "are now ready to operate.\n\n"
    "The bot's long period of inactivity was caused by extended maintenance. "
    "It is expected to become active within approximately 30 minutes. We are "
    "developing the bot without any profit motive.\n\n"
    "[Click here to vote and support the project.]"
    "(https://top.gg/bot/1490285331675615262)"
)
...
class MmCommandTree(app_commands.CommandTree):
    """Yapay zeka kanalı ve Top.gg kullanım kapısını merkezi olarak uygular."""
    _AI_KANAL_IZINLI = {"ai-sıfırla", "ai-sifirla", "ai-dil", "sunucu-dil"}
    _TOPGG_UCRETSIZ = {
        "yardım",
        "oy",
        "yetkili-kanal",
...
async def init_db():
    async with db_connect() as db:
        ...
        """)
        await db.commit()
        # ai_kullanici_dil tablosu — kullanıcı bazında AI dil tercihi
        await db.execute("""
            CREATE TABLE IF NOT EXISTS ai_kullanici_dil (
                user_id BIGINT PRIMARY KEY,
                dil     TEXT NOT NULL DEFAULT 'tr'
            )
        """)
        await db.commit()
        # ai_gunluk_kullanim — günlük Top.gg oy kapısı için sayaç
        await db.execute("""
            CREATE TABLE IF NOT EXISTS ai_gunluk_kullanim (
                user_id BIGINT NOT NULL,
                tarih   TEXT    NOT NULL,
    ...
    try:
        ...
        for p in soup.find_all("p"):
            ...
            if not raw or "Value:" not in raw:
                continue
            # "Name - Value: X" bloklarını bul.
            # [^-\n:]+? → isim kısmı (tire/iki nokta/satır sonu içermez, tembel)
            # Tembel eşleşme " - Value:" ile sonlanana kadar uzar → tam isim yakalanır.
...
def _is_chroma_item(name: str) -> bool:
    """mm2checker.com'da chroma eşyalar 'C. X' veya 'Chroma X' formatındadır."""
    n = name.lower().strip()
    return n.startswith("c. ") or "chroma" in n

def _normalize_chroma(s: str) -> str:
    """Karşılaştırma için 'C. ' → 'chroma ' ve 'chroma ' → 'chroma ' normalleştirir."""
    s = s.lower().strip()
    if s.startswith("c. "):
        s = "chroma " + s[3:]
    return s
...
def fuzzy_find(query: str, results: list, threshold: int = 55) -> dict | None:
    if not results:
        return None
    query_n = _normalize_chroma(query)
    query_has_chroma = "chroma" in query_n
    best, best_score = None, 0
    for item in results:
        name_n = _normalize_chroma(item["name"])
        score = max(
            fuzz.token_sort_ratio(query_n, name_n),
            fuzz.partial_ratio(query_n, name_n)
        )
        # Sorgu "chroma" içermiyorsa chroma eşyaları cezalandır
        if not query_has_chroma and "chroma" in name_n:
            score -= 30
        if score > best_score:
...
def checker_find(query: str, chroma: bool = False) -> dict | None:
    if not _MM2CHECKER_CACHE:
        return None
    # "C. X" ve "Chroma X" her ikisi de chroma sayılır
    pool = {k: v for k, v in _MM2CHECKER_CACHE.items()
            if _is_chroma_item(v["name"]) == chroma}
    if not pool:
        pool = _MM2CHECKER_CACHE
    # Chroma sorgusunda "chroma X" → "c. X" dönüşümüyle arama yap
    search_q = query
    if chroma:
        q_lower = query.lower().strip()
        if q_lower.startswith("chroma "):
            search_q = "c. " + query[7:]
    return fuzzy_find(search_q, list(pool.values()))
    ...
    async with db_connect() as db:
        ...
        for key, item in all_items.items():
            ...
            if key not in eski:
                ...
                def _to_float(v):
                    try: return float(str(v).split("-")[0].replace(",","").strip())
                    except: return 0.0
                old_f, new_f = _to_float(old_v), _to_float(new_val)
                if old_f != new_f:
                    arrow = "📈" if new_f > old_f else "📉"
                    degisimler.append(
...
async def _find_announcement_channel(
    guild: discord.Guild,
) -> discord.TextChannel | None:
    """Find a named bot-command channel in a guild, without broad fallbacks."""
    if not guild:
        return None

    normalized = {
        channel.name.casefold().replace(" ", "-")
...
async def _announcement_channels() -> list[discord.TextChannel]:
    ...
    for guild in bot.guilds:
        if guild.id == ANNOUNCEMENT_GUILD_ID:
            continue
        channel = await _find_announcement_channel(guild)
        if channel and channel.id not in {item.id for item in channels}:
            channels.append(channel)
    return channels


async def send_maintenance_notice_once():
    """Send the bilingual maintenance notice once per notice version."""
    global _maintenance_notice_sent
    if _maintenance_notice_sent:
        return

    async with _maintenance_notice_lock:
        if _maintenance_notice_sent:
            return
        ...
        for channel in channels:
            try:
                ...
                logger.warning(f"Duyuru kanalına yazma yetkisi yok: {channel.id}")
            except discord.HTTPException as ex:
...
# ─── AI DİL TERCİHİ ──────────────────────────────────────────────────────────
async def _get_ai_dil(user_id: int) -> str | None:
    """Kullanıcının AI dil tercihini döndürür. Kayıt yoksa None (sunucu diline bak)."""
    async with db_connect() as db:
        async with db.execute(
            "SELECT dil FROM ai_kullanici_dil WHERE user_id = ?", (user_id,)
        ) as cur:
            row = await cur.fetchone()
    return row[0] if row else None

async def _set_ai_dil(user_id: int, dil: str):
    async with db_connect() as db:
        await db.execute(
            "INSERT OR REPLACE INTO ai_kullanici_dil (user_id, dil) VALUES (?, ?)",
            (user_id, dil)
        )
        await db.commit()

# ─── SUNUCU DİL TERCİHİ ──────────────────────────────────────────────────────
async def _get_sunucu_dil(guild_id: int) -> str:
    """Sunucunun varsayılan AI dilini döndürür. Varsayılan: 'tr'."""
    if not guild_id:
        return "tr"
    async with db_connect() as db:
        async with db.execute(
            "SELECT sunucu_dil FROM settings WHERE guild_id = ?", (guild_id,)
        ) as cur:
            row = await cur.fetchone()
    return (row[0] or "tr") if row else "tr"

async def _set_sunucu_dil(guild_id: int, dil: str):
    async with db_connect() as db:
        await db.execute(
            "INSERT INTO settings (guild_id, sunucu_dil) VALUES (?, ?) "
            "ON CONFLICT (guild_id) DO UPDATE SET sunucu_dil = excluded.sunucu_dil",
            (guild_id, dil)
        )
        await db.commit()

# ─── GÜNLÜK KULLANIM SAYACI (Top.gg oy kapısı) ───────────────────────────────
async def _gunluk_kullanim_al(user_id: int) -> int:
    bugun = datetime.date.today().isoformat()
    async with db_connect() as db:
        async with db.execute(
            "SELECT kullanim_sayisi FROM ai_gunluk_kullanim WHERE user_id = ? AND tarih = ?",
            (user_id, bugun)
        ) as cur:
            row = await cur.fetchone()
    return row[0] if row else 0

async def _gunluk_kullanim_ekle(user_id: int) -> int:
    """Sayacı 1 artırır, yeni değeri döndürür."""
    bugun = datetime.date.today().isoformat()
    async with db_connect() as db:
        await db.execute(
            "INSERT INTO ai_gunluk_kullanim (user_id, tarih, kullanim_sayisi) VALUES (?, ?, 1) "
            "ON CONFLICT (user_id, tarih) DO UPDATE SET kullanim_sayisi = kullanim_sayisi + 1",
            (user_id, bugun)
        )
        await db.commit()
        async with db.execute(
...
async def _topgg_kapi(interaction: discord.Interaction) -> bool:
    """Oy vermeyen kullanıcıların ücretli/ana özellikleri çalıştırmasını engeller."""
    if interaction.user.id == OWNER_ID:
        return True
    if await _topgg_oy_kontrol(interaction.user.id):
        return True

    bot_id = bot.user.id if bot.user else 0
    vote_url = f"https://top.gg/bot/{bot_id}/vote"
    mesaj = (
        "🗳️ Bu özellikten yararlanmak için önce Top.gg'de bota oy vermelisin.\n\n"
        f"[🔗 Oy vermek için tıkla]({vote_url})\n"
        "Oy verdikten sonra komutu tekrar çalıştır."
    )
...
# /oy
@tree.command(name="oy", description="Top.gg üzerinde bota oy verme bağlantısını açar.")
async def oy(interaction: discord.Interaction):
    bot_id = bot.user.id if bot.user else 0
    vote_url = f"https://top.gg/bot/{bot_id}/vote"
    await interaction.response.send_message(
        f"🗳️ Bota destek olmak için [Top.gg'de oy ver]({vote_url}).",
        ephemeral=True,
    )
...
async def setup_hook():
    """Run one-time startup work; on_ready may fire repeatedly after reconnects."""
    await init_db()
...
class GuildJoinDilSelect(discord.ui.Select):
    """Sunucuya katılım DM'inde sunucu dilini seçmek için kullanılır."""
    def __init__(self, guild_id: int):
        self._guild_id = guild_id
        options = [
            discord.SelectOption(label="🇹🇷 Türkçe",  value="tr", description="Varsayılan dil"),
            ...
            discord.SelectOption(label="🇨🇳 中文",     value="zh"),
        ]
        super().__init__(
            placeholder="Sunucu dilini seç / Choose server language",
            options=options,
            min_values=1,
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await _set_sunucu_dil(self._guild_id, chosen)
        label_map = {
            "tr": "🇹🇷 Türkçe", "en": "🇬🇧 English", "de": "🇩🇪 Deutsch",
            "fr": "🇫🇷 Français", "es": "🇪🇸 Español", "ru": "🇷🇺 Русский",
            "ar": "🇸🇦 العربية", "ja": "🇯🇵 日本語", "ko": "🇰🇷 한국어", "zh": "🇨🇳 中文",
        }
        display = label_map.get(chosen, chosen)
        await interaction.response.edit_message(
            embed=discord.Embed(
                title="✅ Sunucu Dili Güncellendi",
                description=(
                    f"Sunucu dili **{display}** olarak ayarlandı.\n\n"
                    "Kullanıcılar `/ai-dil` ile kendi tercihlerini belirleyebilir.\n"
                    "Yöneticiler `/sunucu-dil` komutu ile değiştirebilir."
                ),
...
class GuildJoinDilView(discord.ui.View):
    def __init__(self, guild_id: int):
        super().__init__(timeout=3600)
        self.add_item(GuildJoinDilSelect(guild_id))


@bot.event
async def on_guild_join(guild: discord.Guild):
    await set_default_status()
    # Global commands are synced once in setup_hook.  Copying them to a guild
    # and also keeping the global version creates duplicate slash entries.
    logger.info(
        f"Yeni sunucu katıldı: {guild.name}; global slash komutları kullanılacak."
    )

    # ── Dil tespiti (kanal adlarına göre) ─────────────────────────────────
    TR_KWDS = {"genel", "merhaba", "kurallar", "duyuru", "sohbet", "yardım",
               "türkçe", "güncel", "tanışma", "topluluk", "bilgi", "destek"}
    EN_KWDS = {"general", "hello", "rules", "announcements", "chat", "welcome",
               "english", "info", "introduction", "community", "help", "support"}
    tr_score = en_score = 0
    for ch in guild.channels:
        ...
        for kw in EN_KWDS:
            if kw in n:
                en_score += 1
    tespit_dil = "tr" if tr_score >= en_score else "en"
    await _set_sunucu_dil(guild.id, tespit_dil)
    ...
    try:
        ...
        if owner:
            if tespit_dil == "tr":
                embed = discord.Embed(
                    title="👋 Merhaba! Mm2 Bot sunucuna katıldı!",
                    description=(
                        f"**{guild.name}** sunucusuna eklendim! 🎉\n\n"
                        "Kanal adlarına bakarak sunucu dilini **🇹🇷 Türkçe** olarak ayarladım.\n"
                        "Değiştirmek için aşağıdaki menüyü kullanabilir ya da\n"
                        "sunucuda `/sunucu-dil` komutunu çalıştırabilirsin.\n\n"
                        "**Hızlı Başlangıç:**\n"
                        "• `/yardım` — tüm komutları gör\n"
                        "• `/ayarlar` — bot ayarları\n"
                        "• `/ai-kanal` — yapay zeka kanalı belirle"
                    ),
                    color=discord.Color.blurple(),
                )
            else:
                embed = discord.Embed(
                    title="👋 Hello! Mm2 Bot has joined your server!",
                    description=(
                        f"I've been added to **{guild.name}**! 🎉\n\n"
                        "Based on your channel names, I set the server language to **🇬🇧 English**.\n"
                        "You can change it using the menu below or by running\n"
                        "`/sunucu-dil` in your server.\n\n"
                        "**Quick Start:**\n"
                        "• `/yardım` — view all commands\n"
                        "• `/ayarlar` — bot settings\n"
                        "• `/ai-kanal` — set AI chat channel"
                    ),
                    color=discord.Color.blurple(),
                )
            view = GuildJoinDilView(guild.id)
            await owner.send(embed=embed, view=view)
    except Exception as ex:
        logger.warning(f"Guild join DM gönderilemedi ({guild.name}): {ex}")
...
class DmEsyaView(discord.ui.View):
    """DM cevabına web sayfası butonu ekler."""
    def __init__(self, url: str, label: str = "🌐 Web Sayfasında Gör"):
...
async def handle_dm_mm2(message: discord.Message):
    """DM'de gelen mesajı MM2 eşya adı olarak işler; embed + web butonu döner."""
    esya = message.content.strip()
    if not esya:
        return

    # Çok kısa veya sayı ise yoksay
    if len(esya) < 2:
        return

    # Chroma otomatik tespit — "chroma heart wand" veya "c. heart wand"
    esya_lower = esya.lower()
    is_chroma = esya_lower.startswith("chroma ") or esya_lower.startswith("c. ") or " chroma" in esya_lower
    if esya_lower.startswith("chroma "):
        esya_temiz = esya[7:].strip()
    elif is_chroma:
        esya_temiz = re.sub(r'\bchroma\b', '', esya, flags=re.IGNORECASE).strip(" .")
    else:
        esya_temiz = esya
    search_term = f"chroma {esya_temiz}" if is_chroma else esya_temiz

    async with message.channel.typing():
        # Paralel sorgula
        results_task      = search_mm2values(search_term)
        sv_task           = fetch_supremevalues(search_term)
        results, sv_data  = await asyncio.gather(results_task, sv_task, return_exceptions=True)
        if isinstance(results, Exception):
            results = []
        if isinstance(sv_data, Exception):
            sv_data = None

        # Chroma / normal filtrele
        if is_chroma:
            cr = [r for r in results if "chroma" in r["name"].lower()]
            if cr:
                results = cr
        else:
            results = [r for r in results if "chroma" not in r["name"].lower()] or results

        best_values  = fuzzy_find(search_term, results) or (results[0] if results else None)
        best_checker = checker_find(search_term, chroma=is_chroma)

        # Hiçbir kaynaktan bulunamazsa öneri ver
        if not best_values and not best_checker and not sv_data:
            ...
            return await message.reply(
                f"❌ **{esya}** bulunamadı.\n"
                "Eşya adını tam yazmayı dene (örn: `Biogun`, `Elderwood Scythe`)"
            )

        embed = _build_deger_embed(esya, best_values, best_checker, sv_data=sv_data)

        # Web butonu URL'si: mm2checker.com kategori sayfası#eşya adı
        display_name = (best_checker or best_values or {}).get("name", esya)
        if best_checker and best_checker.get("category"):
            cat  = best_checker["category"]
            anchor = display_name.replace(" ", "")
            url  = f"https://mm2checker.com/{cat}.html#{anchor}"
            label = f"🌐 mm2checker.com — {display_name}"
        elif best_values:
            url   = f"https://mm2values.com/?search={urllib.parse.quote(display_name)}"
            label = f"🌐 mm2values.com — {display_name}"
        else:
            url   = f"https://mm2values.com/?search={urllib.parse.quote(display_name)}"
            label = f"🌐 mm2values.com — {display_name}"

        view = DmEsyaView(url=url, label=label[:80])
...
async def on_message(message: discord.Message):
    ...
    # ── DM'de gelen mesajlar → MM2 eşya sorgusu ──────────────────────────
    if not message.guild:
        if message.content.strip():
            await handle_dm_mm2(message)
        return
...
# ── Discord görselini base64'e çevir (CDN URL'leri AI modelleri tarafından açılamaz) ──
async def _gorsel_base64_indir(url: str) -> str | None:
    """Discord CDN görselini indir ve base64 data URL döndür."""
    try:
        async with aiohttp.ClientSession() as s:
        ...
        logger.warning(f"Görsel indirme başarısız: {ex}")
    return None
...
# ── Bot kütüphanesi — yerel bilgi deposu ─────────────────────────────────────
...
async def _handle_ai_message_inner(message: discord.Message):
    ...
    providers = await _ai_providers()
    if not providers:
        logger.warning("AI: Groq API anahtarı veya kullanılabilir model yok.")
        await message.reply(
            "Yapay zekâ şu anda yapılandırılmamış. Lütfen bot yöneticisi "
            "`GROQ_API_KEY` ayarını ve Groq model erişimini kontrol etsin."
        )
        return
    ...
    # ── Top.gg günlük kullanım ve oy kapısı ────────────────────────────────
    if user_id != OWNER_ID:
        gunluk = await _gunluk_kullanim_al(user_id)
        if gunluk >= TOPGG_GUNLUK_LIMIT:
            oy_verdi = await _topgg_oy_kontrol(user_id)
            if not oy_verdi:
                bot_id = bot.user.id if bot.user else 0
                vote_url = f"https://top.gg/bot/{bot_id}/vote"
                em = discord.Embed(
                    title="🗳️ Günlük Kullanım Hakkın Doldu",
                    description=(
                        f"Bugün **{TOPGG_GUNLUK_LIMIT} mesaj** hakkını kullandın.\n\n"
                        f"Devam etmek için **Top.gg'de bota oy ver** — "
                        f"oy verdikten sonra limite takılmadan konuşabilirsin! 🚀\n\n"
                        f"[🔗 **Oy vermek için tıkla**]({vote_url})\n\n"
                        f"*Oy verdikten sonra bu mesajı tekrar yaz.*"
                    ),
                    color=discord.Color.orange(),
                )
                await message.reply(embed=em)
                return
    await _gunluk_kullanim_ekle(user_id)
    ...
    _INJECT_RE = re.compile(
        r'(discord\.gg/|bot\.run\s*\(|TOKEN\s*=|BOT_TOKEN|CLIENT_ID\s*=|'
        r'import\s+discord.*bot\s*=\s*commands\.Bot|@bot\.command|ctx\.send)',
        re.I | re.DOTALL
    )
    if _INJECT_RE.search(message.content):
        await message.reply("⚠️ Bu mesajı işleyemiyorum.")
        return

    # Kullanıcının dil tercihini al — yoksa sunucu diline bak, yoksa Türkçe
    kullanici_dil = await _get_ai_dil(user_id) or await _get_sunucu_dil(guild_id)
    ...
    # Görsel analiz varsa kullanıcıya hemen bildir, beklemesin
    gorsel_bekle_mesaji: discord.Message | None = None
    if gorsel_eki_url:
        gorsel_bekle_mesaji = await message.reply("🔍 Görsel analiz ediyorum, bir saniye...")
    ...
    async with message.channel.typing():
        ...
        sistem = _ai_sistem_prompt_olustur(profil_bilgisi, stil=stil, dil=kullanici_dil)

        if doviz_veri:
            sistem += f"\n=== CANLI DÖVİZ ({_tarih_tr}) ===\n{doviz_veri}\n"
        if web_sonucu:
            sistem += f"\n=== WEB ARAMA ({_tarih_tr}) ===\n{web_sonucu[:3000]}\n"
        # Uyarıyı YALNIZCA bu mesajda gerçekten anlık veri gerekiyorsa ekle.
        # Takip soruları (emin misin, doğru mu vb.) için ekleme — AI geçmişi kullansın.
        anlik_veri_gerekli = doviz_gerekli or any(
            k in metin_lower for k in (
                "skor", "puan", "maç", "haber", "güncel", "bugün", "şu an",
                "son dakika", "kaç tl", "kaç para", "fiyat", "borsa",
            )
        )
        if anlik_veri_gerekli and (not web_sonucu or "bulunamadı" in (web_sonucu or "")):
            sistem += (
        ...
        # Görsel eki varsa Discord CDN'den base64'e indir (AI modelleri CDN URL'lerini açamaz)
        gorsel_base64: str | None = None
        if gorsel_eki_url:
            gorsel_base64 = await _gorsel_base64_indir(gorsel_eki_url)

        # Görsel eki içeren user mesajı — multimodal format (base64 ile)
        if gorsel_base64:
            kullanici_mesaj_icerik: object = [
                {"type": "text",      "text": kullanici_mesaj[:2000]},
                {"type": "image_url", "image_url": {"url": gorsel_base64}},
            ]
        else:
        ...
        for api_key, base_url, model in providers:
            try:
                # Groq text-only — görsel varsa URL'yi metin olarak belirt
                if gorsel_eki_url:
                    if gorsel_base64:
                        son_mesaj = {"role": "user",
                                     "content": f"{kullanici_mesaj[:2000]}\n[Görsel eklendi: {gorsel_eki_url} — içeriğini Türkçe yorumla]"}
                    else:
                        son_mesaj = {"role": "user",
                                     "content": f"{kullanici_mesaj[:2000]}\n[Görsel indirilemedi]"}
                else:
                ...
                if any(x in hata_str for x in ("429", "404", "400", "quota", "rate_limit",
    ...
    if answer:
        ...
        # Cevabı gönder — görsel analiz için placeholder'ı düzenle, yoksa yeni reply
        # Dile göre footer notu
        if kullanici_dil == "tr":
            footer_notu = "\n-# Sohbetten memnun değilsen, `/ai-sıfırla` ile yapay zekayı sıfırla"
        else:
            footer_notu = "\n-# If you are not satisfied, please use `/ai-sıfırla`"

        parcalar = [answer[i:i + 1990] for i in range(0, len(answer), 1990)]
        if gorsel_bekle_mesaji:
            son_icerik = parcalar[-1] + footer_notu if parcalar else footer_notu
            await gorsel_bekle_mesaji.edit(content=parcalar[0] if len(parcalar) > 1 else son_icerik)
            for j, parca in enumerate(parcalar[1:], 1):
                await message.reply(parca + (footer_notu if j == len(parcalar) - 1 else ""))
                await asyncio.sleep(0.5)
        else:
            for i, parca in enumerate(parcalar):
                son_mi = (i == len(parcalar) - 1)
                await message.reply(parca + (footer_notu if son_mi else ""))
                if i == 0 and len(parcalar) > 1:
                    await asyncio.sleep(0.5)
    elif son_hata:
...
async def deger(interaction: discord.Interaction, esya: str,
    ...
    # Kullanıcı "chroma heart wand" şeklinde yazdıysa otomatik tespit et
    esya_lower = esya.lower().strip()
    auto_chroma = esya_lower.startswith("chroma ") or esya_lower.startswith("c. ") or " chroma" in esya_lower
    if auto_chroma and not esya_lower.startswith("chroma "):
        # "C. Heart Wand" veya "Heart Wand Chroma" → normalleştir
        esya_temiz = re.sub(r'\bchroma\b', '', esya, flags=re.IGNORECASE).strip(" .")
    elif auto_chroma:
        esya_temiz = esya[7:].strip()  # "chroma " prefix'ini at
    else:
        esya_temiz = esya

    # Explicit parametre yoksa auto-detect'e göre karar ver
    if chroma and chroma.value == "evet":
        is_chroma = True
    elif chroma and chroma.value == "hayır":
        is_chroma = False
    else:
        is_chroma = auto_chroma
    ...
    try:
        ...
        if not best_values:
            fallback = await search_mm2values(esya_temiz.split()[0])
            if is_chroma:
                fallback = [r for r in fallback if "chroma" in r["name"].lower()] or fallback
            best_values = fuzzy_find(esya_temiz, fallback) or (fallback[0] if fallback else None)
...
async def yapay_zeka_kur(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    if not GROQ_KEY:
        return await interaction.response.send_message(
            "❌ Yapay zeka için `GROQ_API_KEY` secret'ını Replit Secrets'dan ekle.",
            ephemeral=True)
    async with db_connect() as db:
        await db.execute(
...
@tree.command(name="ai-dil", description="Yapay zekanın seninle konuşacağı dili seç (sadece senin için geçerli).")
@app_commands.describe(dil="Konuşma dilini seç")
@app_commands.choices(dil=_AI_DIL_SECENEKLER)
async def ai_dil(interaction: discord.Interaction, dil: app_commands.Choice[str]):
    await interaction.response.defer(ephemeral=True)
    await _set_ai_dil(interaction.user.id, dil.value)
    em = discord.Embed(
        title="🌐 AI Dil Güncellendi",
        description=(
            f"Artık seninle **{dil.name}** konuşacağım.\n\n"
            "Bu ayar sadece sana özel — sunucudaki diğer kullanıcıları etkilemez.\n"
            "Değiştirmek için komutu tekrar kullanabilirsin."
        ),
        color=discord.Color.blurple()
    )
    await interaction.followup.send(embed=em, ephemeral=True)
...
@tree.command(name="sunucu-dil", description="Botun bu sunucudaki varsayılan dilini ayarlar (yönetici).")
@app_commands.describe(dil="Sunucu için varsayılan dil")
@app_commands.choices(dil=_SUNUCU_DIL_SECENEKLER)
async def sunucu_dil_cmd(interaction: discord.Interaction, dil: app_commands.Choice[str]):
    if not await is_yetkili(interaction):
        await interaction.response.send_message("❌ Bu komutu kullanmak için yetkiniz yok.", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    await log_usage(interaction, "sunucu-dil")
    guild_id = interaction.guild_id or 0
    await _set_sunucu_dil(guild_id, dil.value)
    em = discord.Embed(
        title="🌐 Sunucu Dili Güncellendi",
        description=(
            f"Bu sunucunun varsayılan bot dili **{dil.name}** olarak ayarlandı.\n\n"
            "Kullanıcılar `/ai-dil` komutu ile kendi tercihlerini belirleyebilir "
            "(bireysel tercih, sunucu ayarını geçersiz kılar)."
        ),
        color=discord.Color.green(),
    )
    stats_add(interaction)
