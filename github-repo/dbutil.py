...
# Prefer a bot-specific PostgreSQL URL, then the platform-provided URL.
# SQLite remains available for local development, but its location is
# absolute/configurable so a working directory change cannot silently create a
# second empty database.
DATABASE_URL = (os.getenv("BOT_DATABASE_URL") or os.getenv("DATABASE_URL") or "").strip()
DATA_DIR = os.path.abspath(
    os.getenv("BOT_DATA_DIR")
    or os.path.join(os.path.dirname(os.path.abspath(__file__)), "bot")
)
